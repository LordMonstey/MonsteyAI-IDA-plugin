...
def build_analysis_messages(context: Dict[str, Any], engine_profile: str) -> list:
    ...
    user = {
        ...
        "requirements": [
            ...
            "Do not use generic hook wording like 'confirm call frequency, arguments, and whether this helper sits on a useful path'.",
            "Every trainer_assessment.what_happens_if_hooked item must mention at least one concrete local cue when available: caller, callee, output slot, offset, argument, return value, string, reader width, selector/mode, dirty mask, bitwise operation, or address.",
            "If no concrete cue exists, say that explicitly and recommend moving to the nearest caller/callee instead of pretending the current function is useful.",
